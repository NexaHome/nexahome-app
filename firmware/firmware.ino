...
void checkAndSendEmergency() {
  ...
  if (waterEnabled) {
    // Threshold sensor air diubah menjadi 1800 agar bisa terbaca saat terendam
    String currentWater = (waterVal > 1800 ? "danger" : "low");
...
void setup() {
  ...
  // Menggunakan INPUT_PULLUP untuk mencegah Rain Sensor terdeteksi hujan sembarangan
  pinMode(RAIN_PIN, INPUT_PULLUP); 
  
  pinMode(SMART_LAMP_PIN, OUTPUT); 
  pinMode(AUTO_LAMP_PIN, OUTPUT);
  
  myServo.attach(SERVO_PIN); 
  myServo.write(0);
  
  // Jeda 1 detik agar daya stabil & menghindari Brownout saat Servo menyala
  delay(1000); 
  
  connectWiFi();
}
void loop() {
  fireVal = digitalRead(FIRE_PIN); 
  ...
  if (lightEnabled) {
    if (lightVal > 3800 && autoLampState != "on") { 
      ...
      sendSensor("light", lightVal, "dark");
    }
      ...
      sendSensor("light", lightVal, "bright");
    }
  }
  // LOGIKA OTOMATIS SERVO HUJAN
  ...
  // PENGIRIMAN DATA BERKALA SETIAP 5 DETIK
  if (now - lastSend > 5000) {
    
    // Mengecek apakah sensor Enable sebelum dikirim datanya
    if (fireEnabled)  sendSensor("fire", fireVal, (fireVal == LOW ? "danger" : "safe"));
    if (gasEnabled)   sendSensor("gas", gasVal, (gasVal > 2500 ? "danger" : "normal"));
    if (lightEnabled) sendSensor("light", lightVal, (autoLampState == "on" ? "dark" : "bright"));
    if (waterEnabled) sendSensor("water", waterVal, (waterVal > 1800 ? "danger" : "low"));
    if (rainEnabled)  sendSensor("rain", rainVal, (rainVal == LOW ? "heavy" : "clear"));
